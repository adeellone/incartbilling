...
import { getCompanies, addCompany, updateCompany, Company, CompanyPlan } from "@/lib/firestore/companies";
import { getAllUsers } from "@/lib/firestore/users";
import { db } from "@/lib/firebase";
import { collection, getDocs } from "firebase/firestore";

const PB:Record<CompanyPlan,string>={trial:"badge-yellow",starter:"badge-blue",professional:"badge-green",enterprise:"badge-cyan"};

export default function AdminPage() {
  const { ready, isSuperAdmin } = useReady();
  const [companies,setCompanies]=useState<Company[]>([]);
  const [stats,setStats]=useState({users:0,claims:0,patients:0,providers:0});
  ...
  const load=async()=>{
    ...
    // Super admin: no companyId filter — gets everything
    const [c,u,cl,p,pr]=await Promise.all([
    ...
    setCompanies(c);
    setStats({users:u.length,claims:cl.size,patients:p.size,providers:pr.size});
    setLoading(false);
  };

  useEffect(()=>{ if(ready) load(); },[ready,isSuperAdmin]);
  ...
  if(!ready||loading) return <div className="dash-content" style={{textAlign:"center",paddingTop:80,color:"var(--muted)"}}>Loading platform data...</div>;

  return(
    <div className="dash-content">
      <div style={{marginBottom:28}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(27,111,235,0.12)",border:"1px solid var(--border)",borderRadius:100,padding:"4px 14px",fontSize:12,color:"var(--cyan)",marginBottom:12}}>🌐 Super Admin — Full Platform Access</div>
        <h1 className="sora" style={{fontSize:24,fontWeight:800}}>All Companies</h1>
        <p style={{color:"var(--muted)",fontSize:13,marginTop:4}}>Complete platform overview</p>
      </div>
      <div className="stat-grid" style={{marginBottom:28}}>
        {[
          ...
          <div className="stat-card" key={i}><div className="stat-card-icon">{s.icon}</div><div className="stat-card-label">{s.label}</div><div className="stat-card-val" style={{color:s.color}}>{s.val}</div></div>
        ))}
      </div>
          ...
          <form className="panel" onSubmit={handleAdd}>
            ...
            <div className="form-row">
              ...
              <div className="form-group"><label className="form-label">PLAN</label>
                <select className="form-select" value={form.plan} onChange={set("plan")}>{["trial","starter","professional","enterprise"].map(p=><option key={p}>{p}</option>)}</select>
              </div>
            </div>
      ...
      <div className="data-card">
        {companies.length===0?(<div className="empty"><div className="empty-icon">🏢</div><div className="empty-title">No companies yet</div></div>):(
          <table className="tbl">
                ...
                <td>
                  <div style={{display:"flex",gap:8}}>
                    <select className="form-select" style={{width:"auto",fontSize:12,padding:"4px 8px"}} value={c.plan}
                      onChange={e=>{updateCompany(c.id!,{plan:e.target.value as CompanyPlan});load();}}>
