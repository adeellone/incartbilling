...
interface Payment { id?:string; companyId:string; claimId:string; patientName:string; amount:number; type:"insurance"|"patient"; method:string; notes:string; postedAt?:Timestamp; }
...
export default function PaymentsPage() {
  ...
  const [form,setForm]=useState<Omit<Payment,"id"|"postedAt"|"companyId">>({claimId:"",patientName:"",amount:0,type:"insurance",method:"EFT",notes:""});

  const load=async()=>{
    if(!ready)return;
    const q=queryCompanyId
      ?query(collection(db,"payments"),where("companyId","==",queryCompanyId),orderBy("postedAt","desc"))
      :query(collection(db,"payments"),orderBy("postedAt","desc"));
  ...
  const handleAdd=async(e:React.FormEvent)=>{
    ...
    await addDoc(collection(db,"payments"),{...form,companyId:companyId!,postedAt:serverTimestamp()});
  ...
  return(
    <div className="dash-content">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:28}}>
        <div><h1 className="sora" style={{fontSize:24,fontWeight:800}}>Payments</h1><div style={{color:"var(--muted)",fontSize:13}}>Track collections</div></div>
        ...
        <div style={{marginBottom:24}}>
          <form className="panel" onSubmit={handleAdd}>
            ...
            <div className="form-row">
              ...
              <div className="form-group"><label className="form-label">AMOUNT ($)</label><input className="form-input" type="number" step="0.01" min="0" value={form.amount} onChange={set("amount")} required/></div>
            </div>
            <div className="form-row">
              <div className="form-group"><label className="form-label">TYPE</label>
                <select className="form-select" value={form.type} onChange={set("type")}><option value="insurance">Insurance</option><option value="patient">Patient</option></select>
              </div>
              <div className="form-group"><label className="form-label">METHOD</label>
                <select className="form-select" value={form.method} onChange={set("method")}>{["EFT","Check","Credit Card","Cash","ERA"].map(m=><option key={m}>{m}</option>)}</select>
              </div>
            </div>
            <div className="form-group"><label className="form-label">NOTES</label><input className="form-input" placeholder="Reference number..." value={form.notes} onChange={set("notes")}/></div>
      ...
      <div className="data-card">
        <div className="data-card-header"><span className="data-card-title sora">Payment History</span><span style={{fontSize:13,color:"var(--muted)"}}>{payments.length} payments</span></div>
        {loading?(<div style={{padding:32,textAlign:"center",color:"var(--muted)"}}>Loading...</div>)
        :payments.length===0?(<div className="empty"><div className="empty-icon">💰</div><div className="empty-title">No payments yet</div><button className="btn btn-blue btn-sm" onClick={()=>setShowForm(true)}>Post Payment</button></div>):(
          <table className="tbl">
