...
export default function PaymentsPage() {
  ...
  const [form, setForm] = useState<
    Omit<Payment, "id" | "postedAt" | "companyId">
  >({
    claimId: "",
    patientName: "",
    amount: 0,
  ...
  const set =
    ...
    (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) =>
      setForm((f) => ({
        ...f,
        [k]: k === "amount" ? parseFloat(e.target.value) || 0 : e.target.value,
      }));
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (form.claimId)
      await updateClaim(form.claimId, {
    ...
    <div className="dash-content">
      ...
      >
        {[
          ...
          {
            label: "Insurance",
            val: payments.filter((p) => p.type === "insurance").length,
            color: "var(--blue2)",
          },
          {
            label: "Patient",
            val: payments.filter((p) => p.type === "patient").length,
            color: "var(--cyan)",
          },
        ].map((s, i) => (
            ...
            <div className="form-row">
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={set("method")}
                >
                  {["EFT", "Check", "Credit Card", "Cash", "ERA"].map((m) => (
