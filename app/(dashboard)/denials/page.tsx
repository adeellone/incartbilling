...
const REASONS: Record<string, string> = {
  ...
  "CO-97": "Payment included elsewhere",
  "CO-197": "Precertification absent",
};

const AB: Record<AppealStatus, string> = {
...
const AL: Record<AppealStatus, string> = {
...
export default function DenialsPage() {
  ...
  const [form, setForm] = useState<
    Omit<Denial, "id" | "createdAt" | "companyId">
  >({
    claimId: "",
    patientName: "",
    reasonCode: "CO-4",
    reasonDesc: REASONS["CO-4"],
    deniedAmount: 0,
  ...
  const set =
    ...
    (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) =>
      setForm((f) => ({
        ...f,
        [k]:
          k === "deniedAmount" ? parseFloat(e.target.value) || 0 : e.target.value,
      }));
  ...
  const onReasonChange = (e: React.ChangeEvent<HTMLSelectElement>) =>
    setForm((f) => ({
      ...
      reasonDesc: REASONS[e.target.value] || "",
    }));
    ...
    <div className="dash-content">
      ...
      >
        <div>
          ...
          <div style={{ color: "var(--muted)", fontSize: 13, marginTop: 2 }}>
            Track and appeal denied claims
            <span
              style={{
                marginLeft: 10,
                display: "inline-flex",
      ...
      <div className="stat-grid" style={{ marginBottom: 24 }}>
        {[
          ...
          {
            label: "Appeals Won",
            val: denials.filter((d) => d.appealStatus === "won").length,
            color: "var(--green)",
          },
          {
            label: "In Progress",
            val: denials.filter((d) => d.appealStatus === "in_progress").length,
            color: "var(--yellow)",
          },
        ].map((s, i) => (
                ...
                >
                  ...
                  {claims.map((c) => (
                    <option key={c.id} value={c.id}>
                      {c.patientName} — ${c.totalCharge?.toFixed(2)}
                    </option>
                  ))}
                </select>
              </div>
            ...
            <tbody>
              {denials.map((d) => (
                <tr key={d.id}>
                  ...
                  <td>
                    <span className={`badge ${AB[d.appealStatus]}`}>
                      {AL[d.appealStatus]}
                    </span>
                  </td>
                  <td>
                    <select
                      ...
                      onChange={(e) =>
                        updateAppeal(d.id!, e.target.value as AppealStatus)
                      }
                    >
                      {(Object.keys(AB) as AppealStatus[]).map((s) => (
