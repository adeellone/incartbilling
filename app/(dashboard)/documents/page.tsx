...
import { getDocuments, addDocument, deleteDocument, uploadFile, getDocStatusFromExpiry, DOC_TYPE_LABEL, ProviderDocument, DocumentType } from "@/lib/firestore/documents";
...
export default function DocumentsPage() {
  ...
  const load=async()=>{
    if(!ready)return;
    const [d,p]=await Promise.all([getDocuments(queryCompanyId),getProviders(queryCompanyId)]);
    setDocs(d); setProviders(p); setLoading(false);
  };
  ...
  const handleUpload=async(e:React.FormEvent)=>{
    e.preventDefault(); if(!file||!form.providerId)return;
    ...
    await addDocument({companyId:companyId!,providerId:form.providerId,providerName:form.providerName,type:form.type,name:form.name,fileName:file.name,fileUrl:url,storagePath:path,fileSize:file.size,expiryDate:form.expiryDate,status,notes:form.notes});
  ...
  const handleDelete=async(doc:ProviderDocument)=>{
    ...
    await deleteDocument(doc.id!,doc.storagePath); load();
  };

  const filtered=filter==="all"?docs:docs.filter(d=>filter==="expiring"?d.status==="expiring_soon":filter==="expired"?d.status==="expired":d.type===filter);
  ...
  return(
    <div className="dash-content">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:28}}>
        <div><h1 className="sora" style={{fontSize:24,fontWeight:800}}>Documents</h1><p style={{color:"var(--muted)",fontSize:13,marginTop:4}}>{docs.length} documents stored</p></div>
          ...
          <form className="panel" onSubmit={handleUpload}>
            ...
            <div className="form-row">
              ...
              <div className="form-group"><label className="form-label">TYPE</label>
                <select className="form-select" value={form.type} onChange={set("type")}>{DOC_TYPES.map(t=><option key={t} value={t}>{DOC_TYPE_LABEL[t]}</option>)}</select>
              </div>
            </div>
            <div className="form-row">
            ...
            {uploading&&(
              <div style={{marginBottom:16}}>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:6}}><span style={{color:"var(--muted)"}}>Uploading...</span><span style={{fontWeight:600}}>{progress}%</span></div>
                <div style={{height:6,background:"var(--border)",borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",width:`${progress}%`,background:"var(--blue)",borderRadius:3,transition:"width 0.2s"}}/></div>
              </div>
            )}
      ...
      <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}>
        {[{val:"all",label:`All (${docs.length})`},{val:"expiring",label:`⚠️ Expiring (${expiring})`},{val:"expired",label:`⛔ Expired (${expired})`},...DOC_TYPES.map(t=>({val:t,label:DOC_TYPE_LABEL[t]}))].map(f=>(
      ...
      <div className="data-card">
        ...
        :filtered.length===0?(<div className="empty"><div className="empty-icon">📄</div><div className="empty-title">No documents found</div><button className="btn btn-blue btn-sm" onClick={()=>setShowForm(true)}>Upload Document</button></div>):(
          <table className="tbl">
            ...
            <tbody>{filtered.map(d=>(
              <tr key={d.id}>
                ...
                <td><div style={{display:"flex",gap:8}}><a href={d.fileUrl} target="_blank" rel="noreferrer" className="btn btn-ghost btn-sm">View</a><button className="btn btn-danger btn-sm" onClick={()=>handleDelete(d)}>Del</button></div></td>
              </tr>
            ))}</tbody>
          </table>
        )}
