...
export default function DocumentsPage() {
  ...
  const [form, setForm] = useState({
    providerId:"", providerName:"", type:"license" as DocumentType,
    name:"", expiryDate:"", notes:"",
  });
  const [file, setFile] = useState<File|null>(null);

  const load = async () => {
    const [d, p] = await Promise.all([
      getDocuments(companyId||undefined),
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    await addDocument({
      companyId: companyId!, providerId: form.providerId, providerName: form.providerName,
      type: form.type, name: form.name, fileName: file.name,
      fileUrl: url, storagePath: path, fileSize: file.size,
      expiryDate: form.expiryDate, status, notes: form.notes,
    });
  ...
  const filtered = filter === "all" ? docs : docs.filter(d =>
    filter === "expiring" ? d.status === "expiring_soon" :
    filter === "expired"  ? d.status === "expired" : d.type === filter
  );
    ...
    <div className="dash-content">
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:28 }}>
        <div>
          ...
          <p style={{ color:"var(--muted)", fontSize:13, marginTop:4 }}>{docs.length} documents stored</p>
        </div>
      ...
      {(expiring > 0 || expired > 0) && (
        <div style={{ background:"rgba(255,77,106,0.08)", border:"1px solid rgba(255,77,106,0.25)", borderRadius:12, padding:16, marginBottom:24, display:"flex", gap:24 }}>
          {expired  > 0 && <div><span style={{ color:"var(--red)", fontWeight:700 }}>⛔ {expired} expired</span></div>}
          {expiring > 0 && <div><span style={{ color:"var(--yellow)", fontWeight:700 }}>⚠️ {expiring} expiring soon</span></div>}
      ...
      <div className="stat-grid" style={{ marginBottom:24 }}>
        {[
          { label:"Total Docs",     val:docs.length,  color:"var(--white)"  },
          ...
          <form className="panel" onSubmit={handleUpload}>
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <input className="form-input" placeholder="e.g. CA Medical License 2024" value={form.name} onChange={set("name")} required/>
              </div>
              <div className="form-group">
            ...
            <div className="form-group">
              ...
              <div style={{ border:"2px dashed var(--border)", borderRadius:10, padding:24, textAlign:"center", cursor:"pointer", background:"rgba(27,111,235,0.04)", transition:"border-color 0.2s" }}
            ...
            <div className="form-actions">
              ...
              <button type="submit" className="btn btn-blue" disabled={uploading || !file}>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:20, flexWrap:"wrap" }}>
        {[
          ...
          <button key={f.val} className={`btn btn-sm ${filter===f.val?"btn-blue":"btn-ghost"}`}
      ...
      <div className="data-card">
        <div className="data-card-header">
          <span className="data-card-title sora">{filtered.length} Documents</span>
        </div>
        {loading ? (
          ...
          <div className="empty">
            ...
            <div className="empty-sub">Upload provider documents to track and manage them</div>
            ...
            <tbody>
              {filtered.map(d => (
                <tr key={d.id}>
                  ...
                  <td style={{ fontSize:13, color: d.status==="expired"?"var(--red)":d.status==="expiring_soon"?"var(--yellow)":"var(--muted)" }}>
                    {d.expiryDate || "—"}
                  </td>
                  <td><span className={`badge ${STATUS_BADGE[d.status]}`}>{d.status.replace("_"," ")}</span></td>
