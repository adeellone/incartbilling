  ...
  return(
    <div className="dash-content">
      <div style={{marginBottom:28}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(0,212,138,0.1)",border:"1px solid rgba(0,212,138,0.25)",borderRadius:100,padding:"4px 14px",fontSize:12,color:"var(--green)",marginBottom:12}}>🏥 Provider View — Read Only</div>
        <h1 className="sora" style={{fontSize:24,fontWeight:800}}>Welcome, {user?.displayName?.split(" ")[0]} 👋</h1>
      ...
      <div className="stat-grid" style={{marginBottom:28}}>
        {[
          ...
          {icon:"⚖️",label:"Outstanding",val:loading?"—":fmt(totalCharge-totalPaid),color:"var(--yellow)"},
        ].map((s,i)=><div className="stat-card" key={i}><div className="stat-card-icon">{s.icon}</div><div className="stat-card-label">{s.label}</div><div className="stat-card-val" style={{color:s.color,fontSize:24}}>{s.val}</div></div>)}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:24}}>
        <div className="data-card">
          <div className="data-card-header"><span className="data-card-title sora">Recent Claims</span><a href="/provider-portal/claims" className="btn btn-ghost btn-sm">View all</a></div>
          {loading?(<div style={{padding:32,textAlign:"center",color:"var(--muted)"}}>Loading...</div>)
          :claims.length===0?(<div className="empty"><div className="empty-icon">📋</div><div className="empty-title">No claims yet</div><div className="empty-sub">Your billing team will create claims on your behalf</div></div>):(
            <table className="tbl">
        ...
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          <div className="data-card" style={{padding:24}}>
            ...
            {[
              {label:"Paid",val:claims.filter(c=>c.status==="paid").length,color:"var(--green)"},
              {label:"Denied",val:claims.filter(c=>c.status==="denied").length,color:"var(--red)"},
          ...
          <div className="data-card" style={{padding:24,textAlign:"center"}}>
            ...
            <p style={{fontSize:13,color:"var(--muted)",marginBottom:16}}>Contact your billing team for questions.</p>
