...
const RL: Record<UserRole, string> = {
  superadmin: "Super Admin", company_admin: "Company Admin",
  billing_staff: "Billing Staff", provider: "Provider",
};
...
export default function TeamPage() {
  ...
  const openPermissions = (u: AppUser) => {
    setEditingUser(u);
    setCustomPerms(u.customPermissions || ROLE_PERMISSIONS[u.role]);
  };
    ...
    <div className="dash-content">
      ...
      <div className="data-card">
        ...
        {loading ? (
          ...
          <div className="empty"><div className="empty-icon">👥</div><div className="empty-title">No team members yet</div></div>
        ) : (
          <table className="tbl">
                ...
                <tr key={u.id}>
                  ...
                  <td>
                    <select className="form-select" style={{ width: "auto", fontSize: 12, padding: "4px 8px" }} value={u.role}
                      onChange={async e => { await updateUserProfile(u.uid, { role: e.target.value as UserRole }); }}>
                      <option value="billing_staff">Billing Staff</option>
                      <option value="company_admin">Company Admin</option>
