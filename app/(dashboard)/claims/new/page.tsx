...
export default function NewClaimPage() {
  ...
  // Pre-fill patient name when patients load and patientId is set
  useEffect(() => {
    if (form.patientId && patients.length > 0 && !form.patientName) {
  ...
  const setDx = (i: number) => (e: React.ChangeEvent<HTMLInputElement>) => {
    const a = [...form.diagnosisCodes]; a[i] = e.target.value.toUpperCase();
    setForm(f => ({ ...f, diagnosisCodes: a }));
  };

  const setCpt = (i: number, k: keyof ClaimCode) => (e: React.ChangeEvent<HTMLInputElement>) => {
    const a = [...form.procedureCodes];
    a[i] = { ...a[i], [k]: k === "code" || k === "description" ? e.target.value : parseFloat(e.target.value) || 0 };
    setForm(f => ({ ...f, procedureCodes: a }));
  };
            ...
            <div className="data-card" style={{ padding: 28 }}>
              ...
              <div className="form-row">
                <div className="form-group">
                  <label className="form-label">PATIENT ({patients.length} available)</label>
              ...
              <div className="form-row">
                {form.diagnosisCodes.map((dx, i) => (
                  <div className="form-group" key={i}>
                    ...
                    <input className="form-input" placeholder="e.g. J18.9" value={dx} onChange={setDx(i)}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div className="data-card" style={{ padding: 24 }}>
              ...
              {[
                ...
                ["Diagnoses",  form.diagnosisCodes.filter(Boolean).length],
              ].map(([k, v]) => (
