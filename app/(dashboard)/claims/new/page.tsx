...
const EMPTY_CODE: ClaimCode = { code: "", description: "", units: 1, charge: 0 };
...
export default function NewClaimPage() {
  ...
  const [form, setForm] = useState({
    ...
    diagnosisCodes: ["", "", ""],
    procedureCodes: [{ ...EMPTY_CODE }] as ClaimCode[],
    paidAmount: 0,
    notes: "",
  });
  ...
  const setDx = (i: number) => (e: React.ChangeEvent<HTMLInputElement>) => {
    const arr = [...form.diagnosisCodes];
    arr[i] = e.target.value.toUpperCase();
    setForm((f) => ({ ...f, diagnosisCodes: arr }));
  };

  const setCpt = (i: number, k: keyof ClaimCode) => (e: React.ChangeEvent<HTMLInputElement>) => {
    const arr = [...form.procedureCodes];
    arr[i] = { ...arr[i], [k]: k === "code" || k === "description" ? e.target.value : parseFloat(e.target.value) || 0 };
    setForm((f) => ({ ...f, procedureCodes: arr }));
  };

  const addCptRow = () => setForm((f) => ({ ...f, procedureCodes: [...f.procedureCodes, { ...EMPTY_CODE }] }));
        ...
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 24 }}>
              ...
              <div className="form-row">
                {form.diagnosisCodes.map((dx, i) => (
                  <div className="form-group" key={i}>
                    <label className="form-label">DX {i + 1}</label>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div className="data-card" style={{ padding: 24 }}>
              <div className="sora" style={{ fontSize: 16, fontWeight: 700, marginBottom: 20 }}>Claim Summary</div>
              {[
                ["Patient",      form.patientName  || "—"],
                ["Provider",     form.providerName || "—"],
                ["Service Date", form.serviceDate  || "—"],
                ["Status",       form.status],
                ["Procedures",   form.procedureCodes.length],
                ["Diagnosis",    form.diagnosisCodes.filter(Boolean).length + " codes"],
              ].map(([k, v]) => (
            ...
            <button type="submit" className="btn btn-blue btn-full" disabled={saving} style={{ padding: 16 }}>
              {saving ? "Saving..." : "💾 Save Claim"}
            </button>
            <button type="button" className="btn btn-ghost btn-full" onClick={() => router.push("/claims")}>
              Cancel
            </button>
          </div>
        </div>
