...
const STATUS_BADGE:Record<string,string>={paid:"badge-green",submitted:"badge-blue",denied:"badge-red",draft:"badge-gray",pending:"badge-yellow"};
...
export default function ClaimsPage() {
  ...
  const load=()=>getClaims(companyId||undefined).then(c=>{setClaims(c);setLoading(false);});
  useEffect(()=>{load();},[companyId]);

  useEffect(()=>{
    const q=search.toLowerCase();
    setFiltered(claims.filter(c=>{
      const ms=c.patientName?.toLowerCase().includes(q)||c.providerName?.toLowerCase().includes(q);
      const ms2=status==="all"||c.status===status;
      return ms&&ms2;
    }));
  ...
  return (
    <div className="dash-content">
      ...
      <div className="data-card">
        ...
        {loading?(<div style={{padding:32,textAlign:"center",color:"var(--muted)"}}>Loading claims...</div>)
        :filtered.length===0?(
          <div className="empty">
          ...
          <table className="tbl">
            <thead><tr><th>Patient</th><th>Provider</th><th>Service Date</th><th>Charge</th><th>Paid</th><th>Status</th><th>Actions</th></tr></thead>
            <tbody>
              {filtered.map(c=>(
                <tr key={c.id}>
