...
export default function ReportsPage() {
  ...
  const byProvider=claims.reduce<Record<string,{name:string;count:number;charge:number;paid:number}>>((acc,c)=>{
    ...
    acc[c.providerName].count++; acc[c.providerName].charge+=(c.totalCharge||0); acc[c.providerName].paid+=(c.paidAmount||0);
    return acc;
  },{});
  ...
  return(
    <div className="dash-content">
      <div style={{marginBottom:28}}><h1 className="sora" style={{fontSize:24,fontWeight:800}}>Reports & Analytics</h1><p style={{color:"var(--muted)",fontSize:13,marginTop:4}}>Revenue cycle performance</p></div>
      {loading?(<div style={{textAlign:"center",padding:64,color:"var(--muted)"}}>Loading reports...</div>):(
        <>
          <div className="stat-grid" style={{marginBottom:24}}>
            {[
              ...
              <div className="stat-card" key={i}><div className="stat-card-icon">{s.icon}</div><div className="stat-card-label">{s.label}</div><div className="stat-card-val" style={{fontSize:22,color:s.color}}>{s.val}</div></div>
            ))}
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:24}}>
            ...
            <div className="data-card" style={{padding:24}}>
              ...
              {[
                ...
                {label:"Active Patients",val:pc,target:"—",good:true},
                {label:"Active Providers",val:prc,target:"—",good:true},
              ].map((r,i,arr)=>(
          ...
          <div className="data-card">
            <div className="data-card-header"><span className="data-card-title sora">Claims by Status</span></div>
            {claims.length===0?(<div className="empty"><div className="empty-icon">📊</div><div className="empty-title">No data yet</div></div>):(
              <table className="tbl">
                <thead><tr><th>Status</th><th>Count</th><th>Charge</th><th>Paid</th><th>Rate</th></tr></thead>
                <tbody>{(["paid","submitted","pending","denied","draft"] as ClaimStatus[]).map(s=>{
                  const sc=byStatus(s);
