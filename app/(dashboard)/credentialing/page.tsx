...
const EMPTY_PAYER = { payerName:"", payerId:"", status:"not_started" as const, submittedDate:"", approvedDate:"", notes:"" };
...
export default function CredentialingPage() {
  ...
  const load = async () => {
    const [c, p] = await Promise.all([
      getCredentialings(companyId||undefined),
      getProviders(companyId||undefined),
    ]);
  ...
  const addPayer = () => setForm(f => ({ ...f, payers: [...f.payers, { ...EMPTY_PAYER }] }));
  const removePayer = (i: number) => setForm(f => ({ ...f, payers: f.payers.filter((_, j) => j !== i) }));
  const setPayer = (i: number, k: string, v: string) => setForm(f => {
    const p = [...f.payers]; p[i] = { ...p[i], [k]: v }; return { ...f, payers: p };
  });
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`Delete credentialing for ${name}?`)) return;
    await deleteCredentialing(id); load();
  };

  // Alerts: expiring in 90 days
  const alerts = creds.filter(c => {
    const fields = [c.licenseExpiry, c.deaExpiry, c.malpracticeExpiry, c.boardExpiry];
    return fields.some(f => ["expired","expiring_soon"].includes(getExpiryStatus(f)));
  });
  ...
  return (
    <div className="dash-content">
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:28 }}>
        <div>
          <h1 className="sora" style={{ fontSize:24, fontWeight:800 }}>Credentialing</h1>
          <p style={{ color:"var(--muted)", fontSize:13, marginTop:4 }}>{creds.length} providers credentialed</p>
        </div>
      ...
      {alerts.length > 0 && (
        <div style={{ background:"rgba(255,184,0,0.1)", border:"1px solid rgba(255,184,0,0.3)", borderRadius:12, padding:16, marginBottom:24 }}>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
            {alerts.map(c => <span key={c.id} className="badge badge-yellow">{c.providerName}</span>)}
          </div>
        </div>
      )}
          ...
          <form className="panel" style={{ maxWidth:"100%" }} onSubmit={handleAdd}>
            ...
            <div className="form-row">
              ...
              <div className="form-group">
                ...
                <input className="form-input" value={form.npi} onChange={set("npi")} style={{ fontFamily:"monospace" }} required/>
              </div>
              <div className="form-group">
            ...
            <div className="form-row">
              <div className="form-group"><label className="form-label">LICENSE NUMBER</label><input className="form-input" value={form.licenseNumber} onChange={set("licenseNumber")} style={{ fontFamily:"monospace" }}/></div>
              <div className="form-group"><label className="form-label">STATE</label><input className="form-input" placeholder="e.g. CA, TX, NY" value={form.licenseState} onChange={set("licenseState")}/></div>
              <div className="form-group"><label className="form-label">EXPIRY DATE</label><input className="form-input" type="date" value={form.licenseExpiry} onChange={set("licenseExpiry")}/></div>
            </div>

            <hr className="form-divider"/>
            <div className="form-section">DEA Certificate</div>
            <div className="form-row">
              <div className="form-group"><label className="form-label">DEA NUMBER</label><input className="form-input" value={form.deaNumber} onChange={set("deaNumber")} style={{ fontFamily:"monospace" }}/></div>
            ...
            <hr className="form-divider"/>
            <div className="form-section">Malpractice Insurance</div>
            <div className="form-row">
              ...
              <div className="form-group"><label className="form-label">POLICY NUMBER</label><input className="form-input" value={form.malpracticePolicyNumber} onChange={set("malpracticePolicyNumber")} style={{ fontFamily:"monospace" }}/></div>
              <div className="form-group"><label className="form-label">EXPIRY DATE</label><input className="form-input" type="date" value={form.malpracticeExpiry} onChange={set("malpracticeExpiry")}/></div>
            </div>

            <hr className="form-divider"/>
            <div className="form-section">Board Certification</div>
            <div className="form-row">
              <div className="form-group"><label className="form-label">CERTIFICATION</label><input className="form-input" placeholder="e.g. ABIM, ABFM" value={form.boardCertification} onChange={set("boardCertification")}/></div>
              <div className="form-group"><label className="form-label">EXPIRY DATE</label><input className="form-input" type="date" value={form.boardExpiry} onChange={set("boardExpiry")}/></div>
            </div>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr 1fr 1fr auto", gap:12, alignItems:"flex-end" }}>
                  ...
                  <div className="form-group" style={{ margin:0 }}>
                    ...
                    <select className="form-select" value={p.status} onChange={e => setPayer(i,"status",e.target.value)}>
                      {["not_started","submitted","approved","rejected"].map(s=><option key={s}>{s.replace("_"," ")}</option>)}
                    </select>
                  </div>
                  <div className="form-group" style={{ margin:0 }}>
                    <label className="form-label">SUBMITTED</label>
                    <input className="form-input" type="date" value={p.submittedDate} onChange={e => setPayer(i,"submittedDate",e.target.value)}/>
                  </div>
                  <div className="form-group" style={{ margin:0 }}>
                    <label className="form-label">APPROVED</label>
                    <input className="form-input" type="date" value={p.approvedDate} onChange={e => setPayer(i,"approvedDate",e.target.value)}/>
                  </div>
                  <button type="button" className="btn btn-danger btn-sm" onClick={() => removePayer(i)} style={{ marginBottom:0 }}>✕</button>
                </div>
              </div>
            ))}
      ...
      <div className="data-card">
        ...
        {loading ? (
          ...
          <div className="empty">
            ...
            <div className="empty-title">No credentialing records</div>
            <div className="empty-sub">Add credentialing profiles for your providers</div>
            <button className="btn btn-blue btn-sm" onClick={() => setShowForm(true)}>Start Credentialing</button>
          </div>
        ) : (
          <table className="tbl">
            <thead>
              <tr><th>Provider</th><th>NPI</th><th>License</th><th>DEA</th><th>Malpractice</th><th>Payers</th><th>Status</th><th>Actions</th></tr>
            </thead>
            <tbody>
              {creds.map(c => (
                <tr key={c.id}>
                  ...
                  <td>
                    <div style={{ display:"flex", gap:4, flexWrap:"wrap" }}>
                      {c.payers?.slice(0,2).map((p,i) => (
                        <span key={i} className={`badge ${p.status==="approved"?"badge-green":p.status==="submitted"?"badge-yellow":"badge-gray"}`} style={{ fontSize:10 }}>
                          {p.payerName?.split(" ")[0]}
                        </span>
                      ))}
                  ...
                  <td>
                    <div style={{ display:"flex", gap:8 }}>
                      <a href={`/credentialing/${c.id}`} className="btn btn-ghost btn-sm">View</a>
                      <button className="btn btn-danger btn-sm" onClick={() => handleDelete(c.id!, c.providerName)}>Del</button>
                    </div>
                  </td>
                </tr>
              ))}
