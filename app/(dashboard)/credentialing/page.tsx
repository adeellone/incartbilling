...
import {
  addCredentialing, deleteCredentialing,
  Credentialing, CredentialingStatus,
  getExpiryStatus, daysUntilExpiry, PayerEnrollment,
} from "@/lib/firestore/credentialing";
import { Provider } from "@/lib/firestore/providers";

const SB: Record<CredentialingStatus, string> = {
...
const EP: PayerEnrollment = { payerName: "", payerId: "", status: "not_started", submittedDate: "", approvedDate: "", notes: "" };
...
export default function CredentialingPage() {
  ...
  const alerts = creds.filter(c =>
    [c.licenseExpiry, c.deaExpiry, c.malpracticeExpiry, c.boardExpiry]
      .some(f => ["expired", "expiring_soon"].includes(getExpiryStatus(f)))
  );
  ...
  const setPayer    = (i: number, k: string, v: string) =>
    setForm(f => { const p = [...f.payers]; p[i] = { ...p[i], [k]: v }; return { ...f, payers: p }; });
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
        <div>
          ...
          <p style={{ color: "var(--muted)", fontSize: 13, marginTop: 4 }}>
            {loading ? "Loading..." : `${creds.length} providers credentialed`}
          ...
          <form className="panel" style={{ maxWidth: "100%" }} onSubmit={handleAdd}>
            ...
            <div className="form-row">
              <div className="form-group">
                <label className="form-label">SELECT PROVIDER ({providers.length} available)</label>
      ...
      <div className="data-card">
        ...
        {loading ? (
          ...
          <div className="empty">
            ...
            {providers.length === 0 && <p style={{ color: "var(--red)", fontSize: 13, marginTop: 8 }}>⚠️ Add providers first</p>}
                ...
                <tr key={c.id}>
                  ...
                  <td>
                    <div style={{ display: "flex", gap: 8 }}>
                      ...
                      <button className="btn btn-danger btn-sm" onClick={() => { if (confirm("Delete?")) deleteCredentialing(c.id!); }}>Del</button>
                    </div>
                  </td>
                </tr>
              ))}
