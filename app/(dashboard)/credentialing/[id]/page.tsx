...
import {
  updateCredentialing, Credentialing, CredentialingStatus,
  getExpiryStatus, daysUntilExpiry, PayerEnrollment,
} from "@/lib/firestore/credentialing";
import { ProviderDocument } from "@/lib/firestore/documents";
...
export default function CredentialingDetailPage() {
  ...
  if (!cred) return (
    <div className="dash-content" style={{ textAlign: "center", paddingTop: 80, color: "var(--muted)" }}>Loading credentialing record...</div>
  );
  ...
  return (
    <div className="dash-content">
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 28 }}>
        ...
        <select className="form-select" style={{ width: "auto" }} value={cred.status}
          onChange={async e => { await updateCredentialing(id, { status: e.target.value as CredentialingStatus }); }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div className="data-card">
            <div className="data-card-header">
              ...
              <button className="btn btn-ghost btn-sm" onClick={() => editing ? setEditing(false) : startEdit()}>{editing ? "Cancel" : "Edit"}</button>
            </div>
            ...
            {addingPayer && (
              <div style={{ padding: "16px 24px", borderBottom: "1px solid var(--border)" }}>
                <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr auto", gap: 10, alignItems: "flex-end" }}>
                  ...
                  <div className="form-group" style={{ margin: 0 }}><label className="form-label">SUBMITTED</label><input className="form-input" type="date" value={newPayer.submittedDate} onChange={e => setNewPayer(p => ({ ...p, submittedDate: e.target.value }))} /></div>
                  <div className="form-group" style={{ margin: 0 }}><label className="form-label">APPROVED</label><input className="form-input" type="date" value={newPayer.approvedDate} onChange={e => setNewPayer(p => ({ ...p, approvedDate: e.target.value }))} /></div>
                ...
                <tbody>
                  {cred.payers?.map((p, i) => (
                    <tr key={i}>
                      ...
                      <td>
                        <select className="form-select" style={{ width: "auto", fontSize: 12, padding: "4px 8px" }} value={p.status} onChange={e => updatePayerStatus(i, e.target.value)}>
                          {["not_started","submitted","approved","rejected"].map(s => <option key={s}>{s}</option>)}
                        </select>
                      </td>
