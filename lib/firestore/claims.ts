...
export interface Claim {
  id?:string; companyId:string; patientId:string; patientName:string;
  providerId:string; providerName:string; serviceDate:string; status:ClaimStatus;
  diagnosisCodes:string[]; procedureCodes:ClaimCode[];
  totalCharge:number; paidAmount:number; notes:string;
...
export async function deleteClaim(id:string){return deleteDoc(doc(db,COL,id));}
